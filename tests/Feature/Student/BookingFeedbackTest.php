...
it('student can view feedback page', function () {
    $user = User::factory()->create(['role' => 'student']);
    $student = Student::factory()->for($user)->create();

    $this->actingAs($user)
        ->get(route('student.feedback'))
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->component('student/feedback')
            ->has('pending_feedback')
            ->has('recent_feedback')
            ->has('avg_rating')
        );
});

it('feedback page shows pending bookings that need feedback', function () {
    $user = User::factory()->create(['role' => 'student']);
    $student = Student::factory()->for($user)->create();
