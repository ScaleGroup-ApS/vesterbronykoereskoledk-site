...
function SkillCard({ skill, isApproved }: { skill: Skill; isApproved: boolean }) {
    ...
    return (
        ...
        >
            <p className={`text-sm font-semibold ${isPracticed || isApproved ? 'text-foreground' : 'text-muted-foreground'}`}>
                {skill.label}
            </p>
            {isApproved ? (
                ...
                {practiced.length > 0 && (
                    <section className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                            {practiced.map((skill) => (
                                <SkillCard key={skill.key} skill={skill} isApproved={completedSkills.includes(skill.key)} />
                            ))}
                        </div>
                    </section>
                )}
